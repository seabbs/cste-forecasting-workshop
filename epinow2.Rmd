...
```{r, eval = FALSE}
dates <- seq(as.Date("2021-12-31"), as.Date("2022-03-01"), by = "day")
...
oh_covid_hospitalizations |>
  ...
  guides(col = guide_legend(title = "Date of report")) +
...
```{r estimate_delays, eval = FALSE}
est <- oh_covid_hospitalizations_reporting_cdf |>
  filter(date >= as.Date("2021-12-31")) |>
  ...
  # Remove the first two days that never have reports
  map(~mutate(., report_date = max(date))) |>
  estimate_truncation(
    trunc_max = 10,
    chains = 2, iter = 2000,
    verbose = TRUE
...
The generation time is the time between infection of an individual and infection of their infector. Rather than estimating this here instead we use an estimate from the literature.
...
plot_estimates(
  summary(rt_estimates, type = "reported")
)
```
