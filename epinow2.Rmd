...
# use the covidcast_signal function from the covidcast package to read in data
covid_hospitalizations <- covidcast_signal(
      ...
      filter(issue == max(issue)) |> # keep only the most recent version
      select(geo_value, time_value, value) |> # select these columns
...
# Putting it all together to nowcast and estimate $R_t$
